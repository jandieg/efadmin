...
function getDetalleGruposConMiembros($idGrupo) {
    ...
    $funcion_5= "";
        $cuerpo='';
        $objGrupo= new Grupo();
        $resultset= $objGrupo->getMiembrosPorGrupo($idGrupo,'1');
...
function enviarCorreoGrupo($idGrupo, $asunto,  $mensaje, $key) {
    ...
    $destinatarios=array();
    $objGrupos= new Grupo();
    $resultset= $objGrupos->getMiembrosPorGrupo($idGrupo,$key);
    while($row = $resultset->fetch_assoc()) {     
...
function getTablaFiltro($listaGrupos= array(),$tabla_miembros='' ) {
    ...
    $form['form_2'] = array("elemento" => "combo","change" => "getDetalleFiltroGrupo()", "titulo" => "Grupos", "id" => "_grupo", "option" => $listaGrupos);   
    $resultado = str_replace("{fitros}", generadorEtiquetasFiltro($form), generadorFiltro('Filtros','ben_contenedor')); 
    $resultado = str_replace("{cuerpo}", $tabla_miembros, $resultado); 
