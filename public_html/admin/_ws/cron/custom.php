...
$array = array("1","3","4");
if($type=='enrollment'){
$sql = "SELECT sum(mie_ins_valor) AS total FROM miembro_inscripcion WHERE MONTH(mie_ins_fecharegistro)='$month' OR MONTH(mie_ins_fechamodificacion)='$month' AND mie_ins_year='$year' AND estado_cobro_id='2'";
}
if($type=='dues'){
$sql="SELECT miembro.*, miembro_inscripcion.*, SUM(miembro_inscripcion.mie_ins_valor) AS total FROM miembro, miembro_inscripcion WHERE miembro_inscripcion.mie_ins_year='$year' AND MONTH(miembro_inscripcion.mie_ins_fecharegistro)='$month' AND miembro.status_member_id IN (".implode(',', $array).") AND miembro.mie_id = miembro_inscripcion.miembro_id ";
}
if($type=='other'){
...
include("../../../incluidos/db_config/config.php");
		...
		return convert_datetime($sql_date,'literal');

	
}
//echo getInscription_info('32');
...
$extra_logic="MONTH(mie_fecharegistro)<='$month' AND MONTH(mie_fechamodificacion)>'$month'";	
$array = array("1");
...
AND((t1.status_member_id <> 2) AND (t1.categoria_cat_id<>4) OR(t1.mie_fechamodificacion >=$fmonth)))
";
}else{
...
WHERE(((t1.status_member_id = '$status')
AND(t0.mie_ins_fecha_ingreso <= '$xmonth')) 
AND((t1.status_member_id <> 2) OR(t1.mie_fechamodificacion >=$fmonth)))
...
$sql = "SELECT count(mie_id) AS sp_members FROM miembro WHERE mie_fechamodificacion >= '$xmonth' AND mie_fechamodificacion <='$fmonth' AND status_member_id='2' AND categoria_cat_id='4' AND YEAR(mie_fechamodificacion)='$year'";	
}

if(($category=='key')&&($type=='adds')){
$sql = "SELECT count(mie_id) AS sp_members FROM miembro WHERE mie_fechamodificacion >= '$xmonth' AND mie_fechamodificacion <='$fmonth' AND status_member_id='1' AND categoria_cat_id='4' AND YEAR(mie_fechamodificacion)='$year'";	
}
...
//$sql = "SELECT * FROM miembro WHERE mie_fechamodificacion >= '$xmonth' AND mie_fechamodificacion <='$fmonth' AND status_member_id='2'";	
	$sql = "SELECT * FROM miembro WHERE mie_fechamodificacion BETWEEN '$xmonth' AND '$fmonth' AND status_member_id='2' AND categoria_cat_id<>'4' AND YEAR(mie_fechamodificacion)='$year'";

}

if(($type=='adds')&&($cell=='P')){
//	$sql = "SELECT * FROM miembro WHERE mie_fechamodificacion >= '$xmonth' AND mie_fechamodificacion <='$fmonth' AND status_member_id='1'";
	$sql = "SELECT * FROM miembro WHERE mie_fechamodificacion BETWEEN '$xmonth' AND '$fmonth' AND status_member_id='1' AND categoria_cat_id<>'4' AND YEAR(mie_fechamodificacion)='$year'";
