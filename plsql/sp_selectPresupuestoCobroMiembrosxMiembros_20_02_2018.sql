CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_selectPresupuestoCobroMiembrosxMiembros`(IN `_id` INT, IN `_year` INT)
BEGIN   
...
set v_anhoant = year(curdate()) - 1;
  if _year = year(curdate()) then
   (SELECT
    ...
    (SELECT  `mie_ins_valor` FROM `miembro_inscripcion` WHERE miembro_id = miembro.mie_id and estado_cobro_id='1') as 'valor_inscripcion',
    ...
    where miembro.mie_id=_id and  (presupuestocobro.precobro_year=_year or presupuestocobro.precobro_year is null)
   ...
   (SELECT
    ...
    INNER join persona on miembro.Persona_per_id = persona.per_id 
    Left join presupuestocobro on miembro.mie_id = presupuestocobro.miembro_mie_id     
    where miembro.mie_id=_id and  (presupuestocobro.precobro_year=v_anhoant) 
