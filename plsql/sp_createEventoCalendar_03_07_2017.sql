...
IN `_nombre` VARCHAR(100), IN `_generador_id` INT)
BEGIN
...
INSERT INTO evento(	eve_nombre, eve_responsable, eve_todoeldia, eve_fechainicio, eve_fechafin, eve_fecharegistro, eve_fechamodificacion, eve_id_usuario,
                 tipo_evento_id, direccion_id, eve_descripcion, eve_mis_grupos, eve_todos_grupos,generador_id) 
VALUES (_nombre,_responsable,_toeldia,_fi,_ff,_fr,_fr,_usuario,_id_tipo_evento,_id_direccion,_descripcion,_mis_grupos,_todos_grupos, _generador_id);
...
INSERT INTO evento_grupo( evento_eve_id, grupos_gru_id) VALUES (_LID_EVENTO,4);
    ...
    if _lista_grupos <> '' then
        WHILE (LOCATE(',', _lista_grupos) > 0)
        DO
            SET @value = ELT(1, _lista_grupos);
            SET _lista_grupos= SUBSTRING(_lista_grupos, LOCATE(',',_lista_grupos) + 1);
