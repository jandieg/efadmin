CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_createMiembroFromScript`(_id_empresa int(11), _mie_fecharegistro timestamp,
...
precio_esp, mie_fecha_cambio_status, cancelled) 
values (_id_empresa, 1, _mie_fecharegistro, _mie_fecharegistro, 
fc_getGrupoByDesc(_codigo_grupo), _miembro_id, v_per_id, fc_getFlGrupoByDesc(_codigo_grupo), 1, _membresia_id, 
_status_member_id, _mie_observacion, _precio_esp, _mie_fecha_cancelacion, _cancelled);
