CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_selectMiembroFiltros2`(IN `_id` TEXT, 
IN `_key` INT, IN `_permiso` INT, IN `_canceladas` INT, IN _id_sede INT)
BEGIN
...
if _permiso = '' then  
    if _key = '' then  
            ...
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.cancelled = 0 and usuario.sede_id = _id_sede
       ...
       if _key = '1' then  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id) 
            where miembro.grupo_id = _id and miembro.cancelled = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '2' then  
            SELECT  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.forum_usu_id = _id and miembro.cancelled = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '3' then  
            SELECT  Distinct
            ...
            or miembro.empresalocal_emp_id =_id) and miembro.cancelled = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        ...
        if _key = '4' then  
            ...
            and miembro.cancelled = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '5' then  
          if _id = '2' then 
           SELECT  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.cancelled = 1 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
          else 
           SELECT  
            miembro.mie_id,
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where member_status.mem_sta_id = _id and miembro.cancelled = 0 and usuario.sede_id = _id_sede  
            ORDER BY TRIM(miembro.mie_codigo);
          end if;
        ...
        if _key = '6' then  
            ...
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where grupos.agrup in (_id) and miembro.cancelled = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
    else
        if _key = '' then  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.forum_usu_id = _permiso and miembro.cancelled  = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '3' then  
            SELECT  Distinct
            ...
            where miembro.forum_usu_id = _permiso and 
             miembro.empresalocal_emp_id =_id and miembro.cancelled  = 0 and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        ...
        if _key = '6' then  
            ...
            and grupos.agrup in (_id) and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
...
if _permiso = '' then  
    if _key = '' then  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
       if _key = '1' then  
            SELECT  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.grupo_id = _id and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '2' then  
            SELECT  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.forum_usu_id = _id and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '3' then  
            SELECT  Distinct
        ...
        if _key = '5' then  
          ...
          SELECT  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where member_status.mem_sta_id = _id and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
          end if;
        ...
        if _key = '6' then  
            ...
            INNER join grupos on miembro.grupo_id = grupos.gru_id 
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where grupos.agrup in (_id) and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
    else
        if _key = '' then  
            ...
            INNER join member_status on miembro.status_member_id = member_status.mem_sta_id
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id) 
            where miembro.forum_usu_id = _permiso and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
        if _key = '3' then  
            SELECT  Distinct
            ...
            INNER join usuario on (usuario.usu_id = miembro.forum_usu_id)
            where miembro.forum_usu_id = _permiso and 
             miembro.empresalocal_emp_id =_id and usuario.sede_id = _id_sede 
            ORDER BY TRIM(miembro.mie_codigo);
        end if;
