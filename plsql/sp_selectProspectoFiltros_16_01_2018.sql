CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_selectProspectoFiltros`(IN `_id` INT, IN `_key` INT, IN `_esaplicante` INT, IN _sede_id INT)
BEGIN
  if _key = '' then  
    	if _esaplicante = '0' then 
            ...
            inner join usuario on usuario.usu_id= prospecto.forum_usu_id
            where prosp_esaplicante = _esaplicante 
            and usuario.sede_id = _sede_id 
        ...
        if _key = '3' then  
               if _esaplicante = '0' then 
                ...
                (SELECT  `tel_descripcion` FROM `telefono` WHERE Persona_per_id=persona.per_id and tel_tipo= 'M' LIMIT 1 ) as 'movil',
                (SELECT  concat (persona.per_nombre ,' ', persona.per_apellido) as 'nombre_forum'  FROM persona, perfil, usuario 
                    WHERE usuario.usu_id= prospecto.forum_usu_id and usuario.sede_id = _sede_id and usuario.Persona_per_id= persona.per_id and usuario.perfil_per_id= perfil.per_id LIMIT 1) as 'nombre_forum' 
                FROM  prospecto  
                INNER join persona on prospecto.Persona_per_id = persona.per_id
                inner join usuario on usuario.usu_id= prospecto.forum_usu_id                
                 left join member_status on (member_status.mem_sta_id=prospecto.status_member_id)
