CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_createProspecto`(IN `_esaplicante` INT, IN `_propietario` INT, IN `_nombre` VARCHAR(50), IN `_apellido` VARCHAR(50), IN `_titulo` INT, IN `_correo` VARCHAR(50), IN `_correo_2` VARCHAR(50), IN `_telefono` VARCHAR(50), IN `_celular` VARCHAR(50), IN `_correoparticipacion` INT, IN `_fn` DATE, IN `_fuente` INT, IN `_estado_propietario` INT, IN `_id_skype` VARCHAR(50), IN `_id_Twitter` VARCHAR(50), IN `_calle` TEXT, IN `_ciudad` TEXT, IN `_categoria` INT, IN `_id_usuario` VARCHAR(50), IN `_identificacion` VARCHAR(50), IN `_genero` VARCHAR(50), IN `_tipo_p` VARCHAR(50), IN `_lista_hobbies` TEXT, IN `_lista_desafios` TEXT, IN `_status` INT, IN `_observacion` TEXT, IN `_fecharegistro` TIMESTAMP, IN `_codigo` TEXT, IN `_lafuente` TEXT, IN `_id_empresa` TEXT)
BEGIN
        ...
        if _estado_propietario = '0' then
            ...
            INSERT INTO `prospecto`( `fuente_fue_id`, `Persona_per_id`, `forum_usu_id`, `categoria_cat_id`,  
                                    ...
                                    `prosp_codigo`, `prosp_esaplicante`, `status_member_id`, `prosp_observacion`, `prosp_esaplicanteesmiembro`,
                                     `prospecto_id`, `prosp_aprobado`,prosp_esprospectoaplicante,prosp_txtadicional,prosp_fechacambioforum,prosp_empresa) 
