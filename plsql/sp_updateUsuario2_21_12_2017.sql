CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_updateUsuario2`(IN `_id_user` INT, IN `_nombre` VARCHAR(200), IN `_apellido` VARCHAR(200), IN `_tipo` VARCHAR(20), IN `_identificacion` VARCHAR(50), IN `_fechanacimiento` DATE, IN `_genero` VARCHAR(20), IN `_perfil_id` INT, IN `_estado_user` VARCHAR(50), IN `_id_usuario` INT, IN `_correo` VARCHAR(200), IN `_telefono` VARCHAR(50), IN `_celular` VARCHAR(50), IN `_sede` INT, IN `_pais` INT, 
IN `_esposa` VARCHAR(50), IN `_hijos` VARCHAR(50), IN _sede2 varchar(30))
BEGIN    
    UPDATE `usuario` SET `usu_estado`=_estado_user,`perfil_per_id`=_perfil_id, sede_id = _sede2, `per_id_usuario`=_id_usuario,  pais_id = _pais WHERE `usu_id`=_id_user;
