CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_selectForumLeader7ConGrupos`(IN `_id_forum` INT, IN `_id_user` INT)
BEGIN
if _id_forum <> '' then

        SELECT DISTINCT persona.per_id, 
        persona.per_nombre, 
        ...
        join grupos on (grupos.gru_forum = usuario.usu_id)
        WHERE 
