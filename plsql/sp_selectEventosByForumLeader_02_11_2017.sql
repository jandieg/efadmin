CREATE DEFINER=`execforums`@`localhost` PROCEDURE `sp_selectEventosByForumLeader`(IN `_id_user` INT)
BEGIN
...
where miembro.grupo_id in (select gru_id from grupos where gru_forum = _id_user and agrup in ('A')) and miembro.cancelled = 0 
